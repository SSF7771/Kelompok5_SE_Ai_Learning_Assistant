  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const formattedAnswers = Object.keys(selectedAnswers).map(questionId => {
        ...
        const optionIdx = selectedAnswers[questionId];
        const selectedAnswer = question.options[optionIdx];

        return { questionIdx, selectedAnswer };
      });
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="relative h-2 bg-slate-100 rounded-full overflow-hidden">
          <div 
          className="absolute inset-0 bg-linear-to-r from-emerald-500 to-teal-500 rounded-full transition-all duration-500 ease-out"
          style={{
            ...
            return (
              ...
              >
                ...
                <span className={`ml-4 text-sm font-medium transition-colors duration-200 ${
                  isSelected ? "text-emerald-500" : "text-slate-700 group-hover:text-slate-900"
                }`}>
                  {option}
                </span>
      ...
      <div className="mt-0 flex items-center justify-center gap-2 flex-wrap">
        {quiz.questions.map((_, idx) => {
          const isAnsweredQuestion = selectedAnswers.hasOwnProperty(quiz.questions[idx]._id);
          ...
          return (
            ...
            className={`w-8 h-8 rounded-lg font-semibold text-xs transition-all duration-200 ${
              isCurr
              ? "bg-linear-to-r from-emerald-500 to-teal-500 text-white shadow-lg shadow-green-500/25 scale-110"
              : isAnsweredQuestion
              ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-700'
