...
const QuizResultPage = () => {
  ...
  const correctAnswers = detailedResults.filter(r => { 
    return normalize(r.selectedAnswer) === normalize(r.correctAnswer); 
  }).length;
    ...
    <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-4 pt-4">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border-emerald-200 rounded-xl">
              <CheckCircle2 className='w-4 h-4 text-slate-600' strokeWidth={2}/>
              <span className="text-sm text-emerald-600">
                {correctAnswers} Correct
              </span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-rose-50 border-rose-200 rounded-xl">
              <XCircle className='text-sm text-rose-600' strokeWidth={2}/>
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((res, idx) => {
          const userAnswerIdx = res.options.findIndex(opt => normalize(opt) === normalize(res.selectedAnswer));
          const correctAnswerIdx = res.correctAnswer.startsWith("O")
          ? parseInt(res.correctAnswer.substring(1)) - 1
          : res.options.findIndex(opt => normalize(opt) === normalize(res.correctAnswer));
            ...
            >
              ...
              <div className="space-y-3 mb-4">
                  {res.options.map((opt, optIdx) => {
                    const isCorrectOption = optIdx === correctAnswerIdx;
              ...
              {/* EXPLANATION */}
              {res.explanation && (
                <div className="p-4 bg-linear-to-br from-slate-500 to-slate-100/50 border border-slate-200 rounded-xl ">
      ...
      <div className="mt-8 flex justify-center">
        <Link
          to={`/documents/${quiz.document._id}`}
        >
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden">
